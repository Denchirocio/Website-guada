...
function ContactDrawer({ onClose, cursoDefault }: { onClose: () => void; cursoDefault?: string }) {
    ...
    <>
      ...
      <div style={{ position:'fixed', top:0, right:0, height:'100%', width:560, background:'white', zIndex:101, display:'flex', flexDirection:'column', animation:'drawer-in 0.35s cubic-bezier(0.32,0.72,0,1)' }}>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'0 40px', display:'flex', flexDirection:'column', gap:12 }}>
          <input placeholder="Nombre & apellido*" value={form.nombre} onChange={e => setForm({...form, nombre: e.target.value})} style={{...base, display:'block'}} />
          <input placeholder="E-mail*" value={form.email} onChange={e => setForm({...form, email: e.target.value})} style={{...base, display:'block'}} />

          {/* Nivel de japonés */}
          <div style={{ position:'relative' }}>
            <select value={form.nivel} onChange={e => setForm({...form, nivel: e.target.value})}
              style={{...base, display:'block', cursor:'pointer', color: form.nivel ? '#333' : '#aaa'}}>
          ...
          <div style={{ position:'relative' }}>
            <select value={form.curso} onChange={e => setForm({...form, curso: e.target.value})}
              style={{...base, display:'block', cursor:'pointer', color: form.curso ? '#333' : '#aaa'}}>
